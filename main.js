...
(() => {
  ...
  /* ---------- hero: mouse parallax + glass light ---------- */
  const aurora = $("#aurora");
  ...
  if (!reduceMotion && hero && window.matchMedia("(pointer: fine)").matches) {
    ...
    const tick = () => {
      ...
      heroLight.style.setProperty("--mx", `${clx}%`);
      heroLight.style.setProperty("--my", `${cly}%`);
  ...
  const PROJECTS = {
    sonju: {
      ...
      sections: [
        ...
        ["나의 기여", "음성 입력 → 질문 처리 → 정보 검색 → AI 응답 생성으로 이어지는 서비스 플로우 설계에 참여했습니다. 민원 질문을 구조화된 요청으로 변환하는 방법과 응답의 신뢰성 확보에 집중했고, 행정 용어를 모르는 사용자도 쓸 수 있는 접근성 중심 경험을 설계했습니다."],
    ...
    klap: {
      ...
      sections: [
        ...
        ["나의 기여", "문제 정의부터 KLAS의 반복적인 사용자 흐름 분석, 인증 요청·응답 구조 파악, 비공식 API 클라이언트 구현, CLI/TUI 구조 설계, 세션 관리·응답 파싱·출력 포맷·에러 처리까지 전 과정을 직접 진행했습니다."],
    ...
    insite: {
      ...
      sections: [
        ...
        ["나의 기여", "서비스 컨셉과 AI 피드백 플로우 설계에 참여했습니다. 'AI가 생각을 대신하는 게 아니라 안내해야 한다'는 원칙 아래 단계형 힌트 구조를 만들고, 학습자 코드 처리·피드백 생성·학습 기록 저장을 잇는 백엔드–AI 연결 구조를 고민했습니다."],
    ...
    reminiscence: {
      ...
      sections: [
        ...
        ["나의 기여", "문제 정의와 서비스 방향 설계에 참여했습니다. 기억 지원·루틴 케어·정서적 대화라는 세 축으로 프로젝트를 구조화하고, 복잡한 조작 없이 일상을 지원하는 사용자 시나리오를 만들었습니다. 현재도 서비스 구조와 AI 인터랙션 콘셉트를 발전시키고 있습니다."],
    ...
    unipang: {
      ...
      sections: [
        ...
        ["나의 기여", "문제 정의, 서비스 구조 설계, 기능 기획, 사용자 플로우 정리를 맡았습니다. 기획 단계에서부터 공지 노출 수, 클릭률, 저장률, 퀘스트 완료율, 신청 전환율 등 서비스 지표를 함께 설계해 데이터로 개선할 수 있는 구조를 만들었습니다."],
